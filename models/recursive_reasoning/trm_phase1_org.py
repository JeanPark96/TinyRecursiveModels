...
class TRMLocalizerSync(nn.Module):
    ...
    def forward(self, carry: VideoTRM_InnerCarry, batch: Dict[str, torch.Tensor]):
        ...
        reg_offsets = None
        # ---- localization from y ----
        if isinstance(self.loc_head, StartEndFrameHead):
            start_time_logits, end_time_logits = self.loc_head(z_H, T)
            # Apply Mask
            if attention_mask is not None:
                # Use the boolean frame_mask (B, T)
                # Note: attention_mask in prep_inputs was (B, 1, 1, T), use raw 'mask' or 'frame_mask' from batch
                f_mask = batch.get("video_mask") if "video_mask" in batch else batch.get("frame_mask")
                
                if f_mask is not None:
                    neg_inf = float("-inf")
                    start_logits = start_logits.masked_fill(~f_mask, neg_inf)
                    end_logits = end_logits.masked_fill(~f_mask, neg_inf)
            
            time_logits = start_logits
            reg_offsets = end_logits
        else:
            if isinstance(self.loc_head, DotProductRetrievalHead):
                time_logits = self.loc_head(z_H, v)              # (B,T)
                # ---- Dense Prediction ----
            elif isinstance(self.loc_head, TRM_DenseHead):
                ...
                if frame_mask is not None:
                    time_logits = time_logits.masked_fill(frame_mask == 0, float("-inf"))
                
                #print("prediction", time_logits)

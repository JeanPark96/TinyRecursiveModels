...
class Attention_Mask(nn.Module):
    ...
    # FIX: Add attention_mask argument (default None to not break other code)
    def forward(self, cos_sin: CosSin, hidden_states: torch.Tensor, attention_mask: torch.Tensor = None) -> torch.Tensor:
        ...
        # flash attn setup: [Batch, Seq, Head, Dim] -> [Batch, Head, Seq, Dim]
        query, key, value = map(lambda t: einops.rearrange(t, 'B S H D -> B H S D'), (query, key, value)) 
        
        # FIX: Pass attn_mask to scaled_dot_product_attention
        # Note: PyTorch expects attn_mask to be broadcastable to [Batch, Heads, Seq, Seq]
        attn_output = scaled_dot_product_attention(
            query=query, 
            key=key, 
            value=value, 
            attn_mask=attention_mask, # <--- PASSED HERE
